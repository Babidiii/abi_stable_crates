...
macro_rules! shared_fns {
    (
        ...
        impl<T> $erased<T> {
            ...
            pub(super) extern "C" fn sender_count(this: RRef<'_, Self>) -> usize {
                Self::run(this, |this| this.sender_count())
            }

            pub(super) extern "C" fn receiver_count(this: RRef<'_, Self>) -> usize {
                Self::run(this, |this| this.receiver_count())
            }

            // pub(super) extern "C" fn same_channel(this: RRef<'_, Self>) -> bool {
            //     Self::run(this, |this| this.same_channel().into_c())
            // }
        }
    };
}
